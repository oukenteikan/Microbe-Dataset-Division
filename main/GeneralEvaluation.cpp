...
TempResultSet* GeneralEvaluation::rewritingBasedQueryEvaluation(int dep)
{
	...
	for (int i = 0; i < (int)group_pattern_union.size(); i++)
	{
		...
		bool remote = false;
		Varset remote_query_varset;
		string remote_query, remote_result[subserver_num];
			...
			if (triple_pattern.sub_group_pattern[j].type == QueryTree::GroupPattern::SubGroupPattern::Pattern_type)
				if (triple_pattern.getRootPatternBlockID(j) == j)
				{
					...
					{	printf("%s\t%s\t%s\n", 	basic_query[k].subject.value.c_str(),
						...
						if (basic_query[k].subject.value.find("gene") != string::npos)
							remote = true;
						if (basic_query[k].subject.value.find("genome") != string::npos)
							remote = true;
						if (basic_query[k].subject.value.find("protein") != string::npos)
							remote = true;
						if (basic_query[k].object.value.find("gene") != string::npos)
							remote = true;
						if (basic_query[k].object.value.find("genome") != string::npos)
							remote = true;
						if (basic_query[k].object.value.find("protein") != string::npos)
							remote = true;
					}
		...
		for (int j = 0; j < (int)group_pattern->sub_group_pattern.size(); j++)
			if (group_pattern->sub_group_pattern[j].type == QueryTree::GroupPattern::SubGroupPattern::Optional_type)
			{				
				group_pattern->sub_group_pattern[j].optional.getVarset();
				remote_query_varset += group_pattern->sub_group_pattern[j].optional.group_pattern_resultset_maximal_varset;

				for (int k = 0 ; k < (int)group_pattern->sub_group_pattern[j].optional.group_pattern_resultset_maximal_varset.getVarsetSize(); k++)
				{
					if (group_pattern->sub_group_pattern[j].optional.group_pattern_resultset_maximal_varset.vars[k].find("protein") != string::npos)
						remote = true;
					if (group_pattern->sub_group_pattern[j].optional.group_pattern_resultset_maximal_varset.vars[k].find("gene") != string::npos)
						remote = true;
					if (group_pattern->sub_group_pattern[j].optional.group_pattern_resultset_maximal_varset.vars[k].find("genome") != string::npos)
						remote = true;
				}
			}
		...
		long tv_get = Util::get_cur_time();
		printf("after get local query result, used %ld ms.\n", tv_get - tv_handle);

		}
		else
		{
	
			printf("This is a remote query!\n");
			long tv_send = Util::get_cur_time();

			remote_query += "select ";
			for (int j = 0 ; j < (int)remote_query_varset.getVarsetSize(); j ++)
			{
				remote_query += remote_query_varset.vars[j];
				remote_query += " ";
			}
			remote_query += "where ";

			group_pattern->reconstruction(remote_query);

			printf("The remote query is: \n");
			printf("%s", remote_query.c_str());

			string otherserverIP[subserver_num] = {"10.0.82.153", "10.0.82.154", "10.0.82.155", 
						"10.0.82.156", "10.0.82.157", "10.0.82.158", "10.0.82.159", 
						"10.0.82.160", "10.0.82.161", "10.0.82.162"};
			unsigned short otherserverport[subserver_num] = {10821, 10821, 10821, 10821, 10821, 
									10821, 10821, 10821, 10821, 10821};
			Client* subclient[subserver_num];

			for (unsigned short j = 0 ; j < subserver_num ; j ++)
			{
				printf("IP is %s, port is %u.\n", otherserverIP[j].c_str(), otherserverport[j]);
				subclient[j] = new Client(otherserverIP[j], otherserverport[j]);
				int suc = subclient[j]->connectServer();
				while (!suc) {
					sleep(1);
					subclient[j]->connectServer();
				}
				subclient[j]->send(remote_query);
			}
			for (unsigned short j = 0 ; j < subserver_num ; j ++)
			{
				subclient[j]->receiveResponse(remote_result[j]);
				int suc = subclient[j]->disconnectServer();
				delete subclient[j];
			}

			long tv_receive = Util::get_cur_time();
			printf("after receive remote query result, used %ld ms.\n", tv_receive - tv_send);

			for (unsigned short j = 0 ; j < subserver_num ; j ++)
			{
				//put remote_result into sub_result.
				printf("The remote result length from sub %u is: ", j);
				printf("%u\n", remote_result[j].length());	

				TempResultSet *temp = new TempResultSet();
				temp->results.push_back(TempResult());

				stringstream ss(remote_result[j]);
				string str_msg;
				int varnum;
                                int resnum;
                                Varset var_name;
				...
				ss >> varnum;
                                printf("remote result query varnum is %u\n", varnum);
                                for (int k = 0 ; k < varnum ; k ++)
                                {
                                        ss >> str_msg;
                                        var_name.addVar(str_msg);
                                }
                                temp->results[0].str_varset = var_name;
				
				ss >> resnum;
				printf("remote result num is %d\n", resnum);
				getline(ss, str_msg);
				...
				for (int k = 0 ; k < resnum ; k ++)
				{
					...
					temp->results[0].result.push_back(TempResult::ResultPair());
					temp->results[0].result.back().str = v;
				}	
