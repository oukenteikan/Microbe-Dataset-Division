...
void QueryTree::GroupPattern::FilterTree::FilterTreeNode::reconstruction(std::string & remote_query)
{
	if (this->oper_type == Not_type)					remote_query += "!";
	if (this->oper_type == Builtin_regex_type)			remote_query += "REGEX";
	if (this->oper_type == Builtin_str_type)			remote_query += "STR";
	if (this->oper_type == Builtin_isiri_type)			remote_query += "ISIRI";
	if (this->oper_type == Builtin_isuri_type)			remote_query += "ISURI";
	if (this->oper_type == Builtin_isliteral_type)		remote_query += "ISLITERAL";
	if (this->oper_type == Builtin_isnumeric_type)		remote_query += "ISNUMERIC";
	if (this->oper_type == Builtin_lang_type)			remote_query += "LANG";
	if (this->oper_type == Builtin_langmatches_type)	remote_query += "LANGMATCHES";
	if (this->oper_type == Builtin_bound_type)			remote_query += "BOUND";

	if (this->oper_type == Builtin_in_type)
	{
		remote_query += "(";
		if (this->child[0].node_type == FilterTreeChild::String_type)	remote_query += this->child[0].str;
		if (this->child[0].node_type == FilterTreeChild::Tree_type)		this->child[0].node.reconstruction(remote_query);
		remote_query += " IN (";
		for (int i = 1; i < (int)this->child.size(); i++)
		{
			if (i != 1)	remote_query += ", ";
			if (this->child[i].node_type == FilterTreeChild::String_type)	remote_query += this->child[i].str;
			if (this->child[i].node_type == FilterTreeChild::Tree_type)		this->child[i].node.reconstruction(remote_query);
		}
		remote_query += "))";
	...
	if (this->oper_type == Or_type)				remote_query += " || ";
	if (this->oper_type == And_type)			remote_query += " && ";
	if (this->oper_type == Equal_type)			remote_query += " = ";
	if (this->oper_type == NotEqual_type)		remote_query += " != ";
	if (this->oper_type == Less_type)			remote_query += " < ";
	if (this->oper_type == LessOrEqual_type)	remote_query += " <= ";
	if (this->oper_type == Greater_type)		remote_query += " > ";
	if (this->oper_type == GreaterOrEqual_type)	remote_query += " >= ";

	if (this->oper_type == Builtin_regex_type || this->oper_type == Builtin_langmatches_type)	remote_query += ", ";

	if ((int)this->child.size() >= 2)
	{
		if (this->child[1].node_type == FilterTreeChild::String_type)	remote_query += this->child[1].str;
		if (this->child[1].node_type == FilterTreeChild::Tree_type)		this->child[1].node.reconstruction(remote_query);
	}
...
void QueryTree::GroupPattern::reconstruction(std::string & remote_query)
{
	remote_query += "{\n";
	for (int j = 0; j < (int)this->sub_group_pattern.size(); j++)
		if (this->sub_group_pattern[j].type == QueryTree::GroupPattern::SubGroupPattern::Pattern_type)
		{
			remote_query += this->sub_group_pattern[j].pattern.subject.value;
			remote_query += " ";
			remote_query += this->sub_group_pattern[j].pattern.predicate.value;
			remote_query += " ";
			remote_query += this->sub_group_pattern[j].pattern.object.value;
			remote_query += ".\n";					
		}
	for (int j = 0; j < (int)this->sub_group_pattern.size(); j++)
		if (this->sub_group_pattern[j].type == QueryTree::GroupPattern::SubGroupPattern::Bind_type)
		{
			remote_query += "bind(";
			remote_query += this->sub_group_pattern[j].bind.str;
			remote_query += " as ";
			remote_query += this->sub_group_pattern[j].bind.var;
			remote_query += ")\n";
		}

	for (int j = 0; j < (int)this->sub_group_pattern.size(); j++)
		if (this->sub_group_pattern[j].type == QueryTree::GroupPattern::SubGroupPattern::Optional_type)
		{
			remote_query += "optional ";
			this->sub_group_pattern[j].optional.reconstruction(remote_query);
		}

	for (int j = 0; j < (int)this->sub_group_pattern.size(); j++)
		if (this->sub_group_pattern[j].type == QueryTree::GroupPattern::SubGroupPattern::Filter_type)
		{
			remote_query += "FILTER ";
			this->sub_group_pattern[j].filter.root.reconstruction(remote_query);
			remote_query += "\n";
		}
	remote_query += "}\n";
}
