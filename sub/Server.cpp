...
Server::listen_to_main()
{
	while (true)
	{
		...
		string remote_query;
		bool recv_return = new_server_socket.recv(remote_query);
		if (!recv_return)
		{
			cerr << Util::getTimeString() << "receive command from client error. @Server::listen_to_main" << endl;
			continue;
		}

		cout << Util::getTimeString() << "received query: " << endl << remote_query << endl;

		string remote_result;

		//this->query(remote_query, remote_result);
	
		if (this->database == NULL)
		{
			remote_result = "database has not been loaded.";
		}

		FILE* output = NULL;
#ifdef OUTPUT_QUERY_RESULT
		string path = "logs/gsub_query.log";
		output = fopen(path.c_str(), "w");
#endif

		ResultSet res_set;
		int query_ret = this->database->query(remote_query, res_set, output);
		if (output != NULL) 
		{
			fclose(output);
		}
		...
		stringstream ss;
		ss << res_set.select_var_num << "\n";

                for (int i = 0 ; i < res_set.select_var_num ; i ++)
                {
                        ss << res_set.var_name[i] << "\n";
                        printf("%s ", res_set.var_name[i].c_str());
                }
                printf("\n");

		ss << res_set.ansNum << "\n";
		
		for (int i = 0 ; i < res_set.ansNum ; i ++)
			for (int j = 0 ; j < res_set.select_var_num ; j ++)
			{
				if (res_set.answer[i][j] == "") ss << "\n";
				else ss << res_set.answer[i][j] << "\n";
			}

		remote_result = ss.str();
                printf("%d\n", remote_result.length());
		printf("%s", remote_result.c_str());

		this->response(new_server_socket, remote_result);
